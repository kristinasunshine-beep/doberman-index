  ...
  function mount(options) {
    ...
    function bindBloodlineCardInteractions(container) {
      ...
      container.querySelectorAll(".bln-preview-action[data-bloodline-image]").forEach(button => {
        ...
        // Mobile: one completed tap opens the stance image.
        // No pointerdown/pointerup duplication and no second synthetic activation.
        button.addEventListener("touchend", event => {
          lastTouchOpen = Date.now();
          openPreview(event);
        }, { capture:true, passive:false });

        // Desktop, keyboard and browsers without Touch Events.
        button.addEventListener("click", event => {
          event.preventDefault();
          event.stopPropagation();
          if (Date.now() - lastTouchOpen < 900) return;
          openPreview(event);
        }, { capture:true });
      });
    }
