(function (root, factory) {
  ...
  function computeDesktopLayout(nodes, expandedPaths, width, viewportHeight = 0) {
    ...
    allNodes.forEach(node => {
      const anchor = anchors.get(node.path) || node.path;
      const raw = horizontalPad + positions.get(anchor) * usableWidth;
      const half = (widths.get(node.path) || 0) / 2;
      const leftLimit = horizontalPad + half;
      const rightLimit = safeWidth - horizontalPad - half;
      x.set(node.path, Math.min(rightLimit, Math.max(leftLimit, raw)));
    });
  ...
  function hasReservedPhotoSlot(node) {
    return node?.canonicalId === "DI-A-000004";
  }

  function imageNameMarkup(node) {
    const label = escapeHTML(node.name);
    if (!node.image && !hasReservedPhotoSlot(node)) {
      return `<strong class="bln-name-static"><span class="bln-name-text">${label}</span></strong>`;
    }
    return `<strong class="bln-name-static bln-name-has-preview"><span class="bln-name-text">${label}</span><span class="bln-name-arrow" aria-hidden="true">→</span></strong>`;
  }
  ...
  function nodeMarkup(node, tree, expanded, view, eager = false) {
    ...
    const treeAction = expandable
      ...
      : "";
    const reservedPhotoSlot = hasReservedPhotoSlot(node);
    const previewAction = node.image || reservedPhotoSlot
      ? `<button type="button" class="bln-preview-action" ${imageActionAttrs(node, `G${node.gen || ""}`)}${reservedPhotoSlot && !node.image ? ' data-image-slot="true"' : ""} aria-label="${node.image ? "Open full stance image of" : "Open reserved stance image container for"} ${escapeHTML(node.name)}"></button>`
      : "";
    const label = expandable ? "" : ` aria-label="${escapeHTML(node.name)}, mapped pedigree boundary"`;
    return `<article class="bln-node${expandable ? " is-expandable" : " is-boundary"}${repeated ? " is-repeat" : ""}${node.image || reservedPhotoSlot ? " has-image-preview" : ""}${reservedPhotoSlot && !node.image ? " has-empty-photo-slot" : ""}" data-path="${escapeHTML(node.path)}" data-canonical-id="${escapeHTML(node.canonicalId || "")}"${expandable ? ` data-expanded="${open ? "true" : "false"}"` : ""}${label}>${pictureMarkup(node, node.gen, eager, view)}${nodeCopy(node)}${previewAction}${treeAction}</article>`;
  }
  ...
  function subjectMarkup(subject, compact = false) {
    ...
    const nameMarkup = subject.image
      ? `<strong class="bln-name-static bln-name-has-preview"><span class="bln-name-text">${label}</span><span class="bln-name-arrow" aria-hidden="true">→</span></strong>`
      : `<strong class="bln-name-static"><span class="bln-name-text">${label}</span></strong>`;
    const previewAction = subject.image
      ? `<button type="button" class="bln-preview-action" ${imageActionAttrs(subject, "DI")} aria-label="Open full stance image of ${label}"></button>`
      : "";
    return `<article class="bln-node bln-subject${compact ? " is-compact" : ""}${subject.image ? " has-image-preview" : ""}" data-path="" aria-label="${label}, focal Doberman and tree root"><div class="bln-photo is-textual" aria-hidden="true"><span class="bln-generation"><b>DI</b><i>${role}</i></span></div><div class="bln-copy">${nameMarkup}<span class="bln-identity">${escapeHTML([subject.registration, subject.country].filter(Boolean).join(" · ") || subject.recordId || "Doberman Index")}</span></div>${previewAction}</article>`;
  }
  ...
  function mount(options) {
    ...
    function bindBloodlineCardInteractions(container) {
      container.querySelectorAll("[data-bloodline-path]").forEach(button => {
        button.addEventListener("click", event => {
          event.preventDefault();
          event.stopPropagation();
          toggle(button.dataset.bloodlinePath || "", event.detail === 0);
        });
      });
      ...
      container.querySelectorAll(".bln-preview-action[data-bloodline-image]").forEach(button => {
        ...
        button.addEventListener("pointerdown", event => {
          ...
          if (event.pointerType === "touch") {
            touchPress = { id:event.pointerId, x:event.clientX, y:event.clientY };
            return;
          }
        ...
        button.addEventListener("pointerup", event => {
          ...
          if (!touchPress || touchPress.id !== event.pointerId) return;
          const moved = Math.hypot(event.clientX - touchPress.x, event.clientY - touchPress.y) > 8;
          touchPress = null;
          if (moved) return;
          openPreview(event);
        }, { capture:true, passive:false });

        button.addEventListener("pointercancel", () => { touchPress = null; });
    ...
    function renderDesktop(tree) {
      ...
      // Fit the complete pedigree inside the visible Bloodline shell.
      // Horizontal navigation remains available only if a genuinely narrow viewport needs it.
      ...
      // Keep dense G4 cards clearly separated from the row above/below.
      // The previous 2px top inset / 5px generation gap made card borders visually collide.
      const gap = layout.density >= 4 ? 12 : layout.compactHeight ? 9 : [26, 22, 18, 12, 7][layout.density];
      const laneGap = layout.density >= 4 ? 7 : layout.compactHeight ? 6 : [14, 12, 9, 6, 4][layout.density];
      const rootGap = layout.density >= 4 ? 15 : layout.compactHeight ? 14 : [38, 32, 26, 18, 10][layout.density];
      const rowY = new Map();
      let cursor = layout.density >= 4 ? 12 : layout.compactHeight ? 7 : layout.density >= 3 ? 7 : 10;
      ...
      bindBloodlineCardInteractions(desktop);
    }
    ...
    function renderMobile(tree) {
      ...
      bindBloodlineCardInteractions(mobile);
    }
    ...
    async function openImageViewer(action) {
      ...
      if (imageSrc) viewerImage.decode().then(() => raf(syncViewerStageClearance)).catch(() => {});
    ...
    let rootPreviewTouch = null;
    let rootPreviewSuppressClickUntil = 0;

    function rootPreviewAction(target) {
      if (!target || typeof target.closest !== "function") return null;
      if (target.closest("[data-bloodline-path]")) return null;
      if (target.closest("[data-bloodline-action]")) return null;
      const direct = target.closest(".bln-preview-action[data-bloodline-image]");
      if (direct && root.contains(direct)) return direct;
      const card = target.closest(".bln-node.has-image-preview");
      const action = card?.querySelector(".bln-preview-action[data-bloodline-image]") || null;
      return action && root.contains(action) ? action : null;
    }

    function rootOpenPreview(action, event) {
      if (!action) return;
      event.preventDefault();
      event.stopPropagation();
      rootPreviewSuppressClickUntil = Date.now() + 1000;
      openImageViewer(action);
    }

    root.addEventListener("pointerdown", event => {
      if (event.isPrimary === false) return;
      const action = rootPreviewAction(event.target);
      if (!action) return;
      if (event.pointerType === "touch") {
        rootPreviewTouch = { id:event.pointerId, x:event.clientX, y:event.clientY, action };
        return;
      }
      if (event.button !== 0) return;
      rootOpenPreview(action, event);
    }, { capture:true, passive:false });

    root.addEventListener("pointerup", event => {
      if (event.pointerType !== "touch") return;
      if (!rootPreviewTouch || rootPreviewTouch.id !== event.pointerId) return;
      const press = rootPreviewTouch;
      rootPreviewTouch = null;
      if (Math.hypot(event.clientX - press.x, event.clientY - press.y) > 8) return;
      rootOpenPreview(press.action, event);
    }, { capture:true, passive:false });

    root.addEventListener("pointercancel", () => { rootPreviewTouch = null; }, { capture:true });
    ...
    imageViewer.addEventListener("click", event => {
      if (Date.now() < suppressViewerBackdropUntil) return;
