(function (root, factory) {
  ...
  function nodeMarkup(node, tree, expanded, view, eager = false) {
    ...
    const treeAction = expandable
      ...
      : "";
    const previewAction = node.image
      ? `<button type="button" class="bln-preview-action" ${imageActionAttrs(node, `G${node.gen || ""}`)} aria-label="Open full stance image of ${escapeHTML(node.name)}"></button>`
      : "";
