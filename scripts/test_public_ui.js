...
const male = fs.readFileSync(path.join(root, "profiles/male/index.html"), "utf8");
const female = fs.readFileSync(path.join(root, "profiles/female/index.html"), "utf8");
...
function profileHarness(fetch = async () => { throw new Error("No fixture response"); }) {
  ...
  const document = {
    baseURI: "https://example.invalid/profiles/male/",
    getElementById: id => {
