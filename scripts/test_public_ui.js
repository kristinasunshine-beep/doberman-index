...
function profileHarness(fetch = async () => { throw new Error("No fixture response"); }) {
  ...
  const api = vm.runInNewContext(`${source}\n({ buildBloodline, buildRelated, loadPedigreeIdentities, canonicalToProfile, metrics, bloodline, related, heroMetadata, core, setRegistry:value=>registryRecords=value, setProfile:value=>profileData=value });`, {
...
function testPortalRecognition() {
  ...
  const api = vm.runInNewContext(`${normalization}\n${renderer}\n({setRecords:values=>registryRecords=values.map(normalizeRegistryRecord), renderHomepageShowcase});`, {
  ...
  ].map(record => ({ ...record, status: "published", life_status: "alive", hero: `media/${record.record_id}/hero.png`, hero_focal_point: { x: 12, y: 88 }, kennel_name: "SIEMPRE PELIGROSO", location: "Belgrade" }));
  ...
  for (const [i, category] of ["males", "females", "puppies", "kennels"].entries()) {
    ...
    assert.equal(label.textContent, "01 / LIVE");
...
async function main() {
  ...
  assert.match(index, /\.featured-record-copy strong.registered-name\{text-transform:none\}/);
  ...
  console.log("Public UI micro-patch PASS (Search/Portal portrait contain; future Male/Female/Puppy recognition without focal crop; shared editorial names; top-right KPI/mask; real-only pedigree metadata; kennel/hero hierarchy; canonical preserved)");
}
